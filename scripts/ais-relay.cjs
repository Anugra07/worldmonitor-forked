...
function isSupportedRedisRestUrl(rawUrl) {
  if (!rawUrl) return false;
  try {
    const parsed = new URL(rawUrl);
    if (parsed.protocol === 'https:') return true;
    if (parsed.protocol !== 'http:') return false;
    return ['localhost', '127.0.0.1', 'redis-rest'].includes(parsed.hostname);
  } catch {
    return false;
  }
}
const UPSTASH_ENABLED = !!(
  ...
  isSupportedRedisRestUrl(UPSTASH_REDIS_REST_URL)
);
...
const UPSTASH_HTTP_CLIENT = UPSTASH_REDIS_REST_URL.startsWith('http://') ? http : https;

if (UPSTASH_REDIS_REST_URL && !isSupportedRedisRestUrl(UPSTASH_REDIS_REST_URL)) {
  console.warn('[Relay] UPSTASH_REDIS_REST_URL must be https:// or a local http:// Redis REST endpoint — Redis disabled');
}
if (UPSTASH_ENABLED) {
...
function upstashGet(key) {
  return new Promise((resolve) => {
    ...
    const req = UPSTASH_HTTP_CLIENT.request(url, {
      method: 'GET',
